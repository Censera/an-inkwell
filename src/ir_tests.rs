...
fn rejects_non_boolean_logical_operands() {
    ...
    assert!(matches!(builder.logical_and(&first, &second), Err(super::Error::NotBoolean)));
    assert!(matches!(builder.logical_or(&first, &second), Err(super::Error::NotBoolean)));
...
fn builds_bitwise_operations() {
    ...
    assert!(builder.bit_and(&first, &second).unwrap().as_ir().contains("i8 -"));
    assert!(builder.bit_or(&first, &second).unwrap().as_ir().contains("i8 -"));
    assert!(builder.bit_xor(&first, &second).unwrap().as_ir().contains("i8 6"));
    assert!(builder.bit_not(&first).unwrap().as_ir().contains("i8 -11"));
    assert!(builder.shl(&first, &Value::integer(&integer, 1, false)).unwrap().as_ir().contains("i8 20"));
    assert!(builder.lshr(&second, &Value::integer(&integer, 2, false)).unwrap().as_ir().contains("i8 3"));
    assert!(builder.ashr(&Value::integer(&integer, 0b1111_0000, false), &Value::integer(&integer, 2, false)).unwrap().as_ir().contains("i8 60"));
}
...
fn builds_cast_operations() {
    ...
    assert!(builder.trunc(&Value::integer(&i32, 5, false), &i8).unwrap().as_ir().contains("i8 5"));
    assert!(builder.zext(&integer, &i32).unwrap().as_ir().contains("i32 5"));
    assert!(builder.sext(&signed_integer, &i32).unwrap().as_ir().contains("i32 -6"));
    assert!(builder.fptoui(&float, &i32).unwrap().as_ir().contains("i32 5"));
    assert!(builder.fptosi(&float, &i32).unwrap().as_ir().contains("i32 5"));
    assert!(builder.uitofp(&integer, &f32).unwrap().as_ir().contains("float 5.000000e+00"));
    assert!(builder.sitofp(&signed_integer, &f32).unwrap().as_ir().contains("float -6.000000e+00"));
    assert!(builder.fptrunc(&Value::float(&f64, 5.5), &f32).unwrap().as_ir().contains("float 5.500000e+00"));
    assert!(builder.fpext(&float, &f64).unwrap().as_ir().contains("double 5.500000e+00"));
    assert!(builder.ptrtoint(&Value::null(&pointer).unwrap(), &i32).unwrap().as_ir().contains("i32 0"));
    assert!(builder.inttoptr(&Value::integer(&i32, 0, false), &pointer).unwrap().as_ir().contains("ptr null"));
}
...
fn rejects_casts_from_different_contexts() {
    ...
    assert!(matches!(builder.zext(&value, &second_type), Err(super::Error::DifferentContext)));
}
